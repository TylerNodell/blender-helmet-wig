...
def generate_head_mesh(
    ...
    # --- Occipital bump ---
    # The back of the head has a bump at roughly 20-30% of head height.
    # This adds extra backward extent in the lower-back region.
    occipital_bump_height = 0.25  # fraction of height where bump peaks
    occipital_bump_strength = half_depth * 0.08  # how far it protrudes
    ...
    for j in range(v_segments):
        ...
        # --- Occipital bump: extra back depth at lower heights ---
        bump_influence = math.exp(-((t - occipital_bump_height) ** 2) / (2.0 * 0.08 ** 2))
        local_back += occipital_bump_strength * bump_influence

        # --- Forehead flattening ---
        # Compresses the front Y extent to make forehead more vertical.
        # Very strong at bottom, fades out by mid-height.
        if t < 0.4:
        ...
        for i in range(u_segments):
            ...
            # --- Y (front-back) ---
            if sin_u >= 0:
                # Front: apply forehead flattening
                y = local_front * sin_u * (1.0 - forehead_flatten * front_factor)
            else:
                # Back: full occiput depth (sin_u is negative)
                y = local_back * sin_u
